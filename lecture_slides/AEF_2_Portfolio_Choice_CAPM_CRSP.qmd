...
tibble(
  ...
  geom_point(size = 1) +
  geom_point(
    data = res |> filter(a %in% c(0)),
    size = 4, alpha = 0
  ) +
  labs(
    x = "Annualized standard deviation",
    y = "Annualized expected return",
    title = "Returns of Dow Jones 30 constituents"
  )  +
...
tibble(
  ...
  annotate(
    geom = "curve", 
    xend = res |> filter(a %in% c(0))|>pull(sd), 
    yend = res |> filter(a %in% c(0))|>pull(mu), 
    x = res |> filter(a %in% c(0))|>pull(sd), 
    y = 0.2,
    curvature = 0.2,
    arrow = arrow(length = unit(4, "mm"))
  ) +
  annotate(geom = "text", x = res |> filter(a %in% c(0))|>pull(sd), y = 0.2, label = "frac(~Sigma^{-1}~iota,~iota^T ~Sigma^{-1}~iota)", parse = TRUE, hjust = "left")
