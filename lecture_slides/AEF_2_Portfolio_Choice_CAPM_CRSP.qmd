...
```{r}
prices <- tq_get(c("META", "AAPL", "MSFT"), get = "stock.prices", from = "2000-01-01")

price_matrix <- prices |> 
    group_by(symbol) |> # Net returns per symbol
    transmute(date, return = (adjusted - lag(adjusted)) / lag(adjusted)) |>  
  pivot_wider(names_from = symbol, values_from = return) |> drop_na() |> select(-date) 
...
- Preference for higher expected return and lower volatility $\sigma^{pf} \left(=\sqrt{\text{Var}(r^{pf}_t})\right)$
$$\begin{aligned}
...
## The efficient frontier with R (exercise)
...
- `for` loops provide a way to tell R, “Do *this* for every value of *that*.” In R syntax, this looks like this:
...
- For more details on how to use loops in R, [Chapter 11 of this book](https://rstudio-education.github.io/hopr/loops.html) is a great tool
...
- Note that in *Tidy Finance with R*, we explain how to get CRSP data from the WRDS interface. This does not work on the KU campus!
...
- Monthly processed (!) data is available in `tidy_finance_r.sqlite` file in Absalon
...
- We adjust the market capitalization values for inflation using the Consumer Price Index (CPI)
- CPI data from the Bureau of Labor Statistics website. The data (All Urban Consumer series) are available at [www.bls.gov/cpi/data.htm](www.bls.gov/cpi/data.htm)
...
- *Note*: The `tidy_finance_r.sqlite` data already contains the processed CRSP file with excess and raw returns. For the sake of simplicity, I removed the delisting information (consult the exercises for more details)
...
beta <- tbl(tidy_finance, "beta") |> collect() |> 
  inner_join(crsp_monthly, by = c("month", "permno")) |> drop_na(beta_monthly)
...
beta_portfolios <- beta |> # A stylized example (note time convention in exercise!)
  ...
  summarize(ret = weighted.mean(ret_excess, mktcap_lag), .groups = "drop") # value-weighted performance
