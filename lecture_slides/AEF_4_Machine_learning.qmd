...
ggplot(values, aes(x = lambda, 
  ...
  scale_color_discrete(labels = c("Bias (Squared)", "MSE", "Variance"))
...
- repeat the calculation above for each of these sets $b=1,\dots,K$ and obtain $\hat{\mbox{MSE}}_1(\lambda),\dots, \hat{\mbox{MSPE}}_K(\lambda)$ and compute the average
$$
...
```{r, size = "tiny", out.width='50%', fig.align='center'}
library(rpart)
model <- rpart(
  ret_excess ~ ., 
...
![]("figures/neural_network_one_unit.png"){width=70%}
...
![]("figures/neural_network.jpg"){width=50%}
...
- Despite the computational challenges, implementation in R is not tedious at all: We can use the [API to tensorflow](https://tensorflow.rstudio.com/installation/)
...
### Exercise
- Follow the `Tensorflow` installation steps and study some of [their examples](https://tensorflow.rstudio.com/tutorials/)
...
- Compute the loss $\mathcal{L}\left(\tilde y, y\right)$, e.g. MSE
