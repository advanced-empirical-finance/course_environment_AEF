...
- Teaching assistants: XX (\href{mailto::XX@econ.ku.dk}{XX@econ.ku.dk})
- XX moderates Absalon discussions, but it is **your responsibility** to actively engage in the exchange of ideas, code, and knowledge!
...
- Your TA is there to help you - but it is your responsibility to ask if you have questions

The learning curve is very steep, but remember that you can reach out **anytime** in case something is unclear!

## Mandatory Assignments

- Coding assignments in which we transfer the theory into practice
...
- **Handed-in report: max. 7 pages for .pdf file in total** (font size 12pt, use [this (click here)](https://github.com/advanced-empirical-finance/aef_environment) template)
...
- **Minimum requirement to get feedback:** Attempt to solve every task. The code needs to run without any error or interruption so that your peers can focus on the content instead of fixing your bugs
...
- You will hand in a .pdf report and a .R or .qmd code script that replicates all results in your report
...
2. Coding experience (General, R, tidyverse)?
...
- Instead of focusing on sharpening your students' coding skills within Python *or* R, I want to make a case for tidy coding
...
- Both are free to download and use / open-source (Users can expand the functionalities through add-ons called packages) / Data processing is easy / Data visualization tools are pervasive / It is easy to share your (reproducible) output using Quarto
- Python and R are nowadays the de-facto standard tools in Finance (+ you can run Python scripts from within R and vice versa)
- With Quarto you can embed code directly into the analysis
...
- [Install R / Install RStudio](https://posit.co/download/rstudio-desktop/#download) / [Install Quarto](https://quarto.org/docs/get-started/) / [clone the course repo](https://github.com/advanced-empirical-finance/aef_environment) / `install.packages(reticulate)` and install Python `reticulate: reticulate::install_python(version="3.10.11", force = TRUE)` / `renv::restore()` fetch all packages required for this course
...
joint_prices <- left_join(prices, prices_sp500, by = "date")
...
p + labs(x = "Date", y ="Volume (USD)", title = "Daily volume") +
  facet_wrap(~symbol, scales = "free_x") + theme_light() +
...
## Still hard to read Python code?
...
1. If you do not know what a command, e.g., `cov()` is doing, type `?cov` in the console
1. Search online, e.g. *R cov* - google will know
...
1. For more general *R* and *Python* exercises, make use of free access to [datacamp](www.datacamp.com)
