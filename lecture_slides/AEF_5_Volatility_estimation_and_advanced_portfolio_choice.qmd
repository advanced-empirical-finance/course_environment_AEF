...
## Scope - what can we say about $\Sigma$?

![]("figures/storm.jpg"){width=50% fig-align="center"}
...
- $\hat\sigma_t^2$ is also a crucial component for *option pricing* - recall Black-Scholes
...
- Describes the dependence of $\varepsilon_t$ by an ARCH(m) model 
...
- We define the VaR (Value-at-risk) of a financial position over a given time horizon $l$ with tail
probability $p$ as $$p = \text{Pr}\left(L(l) \geq VaR\right) = 1 - \text{Pr}\left(L(l) \leq VaR\right)$$
where $L$ is a loss function, e.g. in terms of USD
- probability that the position holder would encounter a loss greater than or equal to VaR over the time horizon $l$ is $p$
